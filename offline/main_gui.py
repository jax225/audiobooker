...
class AudioBookApp:
    def __init__(self, root):
        ...
        self.voice = tk.StringVar(value="xenia")
    ...
    def create_widgets(self):
        ...
        # ----- ШАГ 1: TXT -> HTML (с ударениями) -----
        step1_frame = ttk.LabelFrame(main_frame, text="Шаг 1: TXT → HTML (с ударениями)", padding="10")
        ...
        voice_combo = ttk.Combobox(settings_frame, textvariable=self.voice, width=20,
                                   values=["aidar", "baya", "kseniya", "xenia", "eugene"])
    ...
    def create_html(self):
        ...
        def worker():
            success, html_path, msg, count = create_html_from_txt(
                txt, log_callback=self._log_threadsafe
            )
    ...
    def _create_html_done(self, success, html_path, msg, count):
        ...
        if success:
            self.lbl_html_result.config(
                text=f"✅ {msg} ({count} абзацев)", foreground="green"
            )
            self.html_path.set(html_path)
    ...
    def _generation_done(self, success):
        ...
        if success:
            ...
            self.log("⚠️ Генерация не выполнена (модуль TTS — заглушка).")
